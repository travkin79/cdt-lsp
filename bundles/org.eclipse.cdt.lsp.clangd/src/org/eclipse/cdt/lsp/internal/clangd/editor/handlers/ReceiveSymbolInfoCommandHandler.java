...
public class ReceiveSymbolInfoCommandHandler extends AbstractHandler {
	...
	private Object execute(IEditorPart activeEditor, ISelection currentSelection) {
		...
		getUri(innerEditor).ifPresent(fileUri -> {
			...
			try {
				positionParams.setTextDocument(LSPEclipseUtils.toTextDocumentIdentifier(document));
				positionParams.setPosition(LSPEclipseUtils.toPosition(textSelection.getOffset(), document));
			...
			try {
				Optional<SymbolDetails[]> result = LanguageServers.forDocument(document)
						.computeFirst(server -> server instanceof ClangdLanguageServer
								? ((ClangdLanguageServer) server).getSymbolInfo(positionParams)
								: null)
						.get();
				if (result.isPresent()) {
					// TODO return AST
